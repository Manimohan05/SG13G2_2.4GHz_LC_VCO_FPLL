...
# ── Settling window: use only 55 µs – 75 µs ─────────────────────────────────
T_START = 58e-6
T_END   = 73e-6
...
def load_two_signals(filepath, sig_name, downsample=1, t_start=None, t_end=None):
    ...
    # Time slice (start and optional end)
    if t_start is not None:
        idx_s = np.searchsorted(t, t_start)
        t, v = t[idx_s:], v[idx_s:]
    if t_end is not None:
        idx_e = np.searchsorted(t, t_end)
        t, v = t[:idx_e], v[:idx_e]
...
def main():
    ...
    print(f"Loading v(clk_out) (settled window {T_START*1e6:.0f}–{T_END*1e6:.0f} µs, "
          f"downsample=4) ...")
